...
function Carroussel({slidesImg}) {
    ...
    return (
        <div className="carrousel">
            ...
            <div>
                <div>
                    <img src={fleche_gauche} alt='fleche gauche' className={slidesImg.length <= 1 ? "fleche_none" : "fleche_gauche"} onClick={goPrevious}/>
                </div>
                <div className="carrousel_number" >
                    {actualIndex + 1}/{slidesImg.length}
                </div>
                <div>
                    <img src={fleche_droite} alt='fleche droite' className ={slidesImg.length <= 1 ? "fleche_none" : "fleche_droite"} onClick={goNext}/>
                </div>
            </div>
        </div>
    )
